...
Builder.prototype.getScript = function (schema) {
  ...
  _.each(schema.addedTables, function (table, name) {
    ...
    _.each(table.addedIndexes, function (idx, name) {
  ...
  _.each(schema.removedTables, function (table, name) {
    _.each(table.removedIndexes, function (idx, name) {
      scriptParts.push(this._getDropIndexScript(name, idx));
    }, this);
...
Builder.prototype._getTableScript = function (name, definition) {
  ...
  var primaryKeyScript = this._getPrimaryKey(primary);
  if (primaryKeyScript) {
    columnParts.push(primaryKeyScript);
  }
...
Builder.prototype._getColumnScript = function (column, definition) {
  var ret = ['`' + column + '`' + ' ' + (this.dataTypes[definition.type] || definition.type)];
